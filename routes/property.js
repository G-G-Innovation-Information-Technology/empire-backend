...
router.get('/', async (req, res) => {
  try {
    ...
    if (publishing_status) {
      ...
      query.publishing_status = { $nin: ['DRAFT'] };
    }
    ...
    // Sorting logic: prioritize featured + published properties first
    ...
    const properties = await Property.aggregate([
        ...
        $addFields: {
          sortPriority: {
            $cond: [
              ...
              1, // Featured + Published properties get priority 1
              0, // All others get priority 0
            ],
          },
        },
      },
      {
        $sort: {
          sortPriority: -1, // Sort by priority first (descending)
          [sortField]: sortOrder, // Then by the specified field
          createdAt: -1, // Finally by creation date (newest first) as tiebreaker
        },
      },
      { $skip: (page - 1) * limit },
      { $limit: limit * 1 },
